...
def compute_hash(file_path, hash_type):
    try:
        ...
        total_chunks = (file_size // 4096) + (1 if file_size % 4096 else 0)
        
        with open(file_path, 'rb') as f:
            for _ in tqdm(xrange(total_chunks), desc="Computing {0} hash".format(hash_type)):
                chunk = f.read(4096)
                h.update(chunk)
        ...
        if choice == '1':
            ...
            if os.path.isdir(directory):
                ...
                for file_path in file_paths:
                    ...
                    save_scan_log(log_file, file_path, hashes, matches, similarity_scores)
                    scanned_files += 1
                print("\nScanned {0} out of {1} files.".format(scanned_files, total_files))
            else:
